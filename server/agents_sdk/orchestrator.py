...
def build_orchestrator() -> tuple[Agent[OrchestrationContext], dict]:
    ...
    merch_agent = _mk_agent(
        ...
        (
            ...
            "When specifically asked to visualize merch, also include an image-generation prompt wrapped exactly as <IMAGE_PROMPT> ... </IMAGE_PROMPT>, followed by a one-line 'Caption: ...'\n"
        ),
        MerchBrief,
    )
...
async def chat_respond(email: str, history: List[ChatTurn]) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            # Also detect inline image prompt in fallback
            try:
                start_tag = "<IMAGE_PROMPT>"
                end_tag = "</IMAGE_PROMPT>"
